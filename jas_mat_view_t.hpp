...
private:
    ...
    int m_row_size;
    int m_col_size;
    bool m_transposed;      // whether this view is transposed
public:
     ...
     * This is the precondition of reshape_view(): flattening a strided sub-view is not memory order.
     */
    ...
    /**
     * Shape view: reinterpret this view's elements in memory order as (rows, cols); throws if not dense.
     *
     * Only lvalues may call it here either: in `x.view(...).reshape_view(...)` the temporary view dies
     * at the end of the full expression while the reshape view references it -- name it first.
     */
     ...
     *   - transposed    : in storage this is the (col_size x row_size) row-major matrix at that
     *                     offset, handed to BLAS with CblasTrans (requires ld >= row_size)
     * A column-major base, or a base that is itself transposed, cannot; it returns invalid.
     */
 ...
 *   - `mat_reshape_view_t` -> change the shape; elements correspond one to one.
 *
 * Rules:
 *   - `rows * cols` must equal the base element count, else the constructor throws std::invalid_argument;
 *   - indexing follows the **same storage order as the base**: for a row-major base view(i,j) is
 *     flat element `i*cols + j`; for a column-major base it is `j*rows + i` (numpy order='C'/'F');
 *   - it is therefore an **alias**, not a copy: writes through it change the base data (and back);
...
public:
     ...
     * contiguous), so ld = cols; transposed the stored matrix is (cols x rows), satisfying ldb >= K.
     */
    detail::gemm_buffer<val_type> gemm_view() const noexcept
    {
        const detail::gemm_buffer<val_type> base = m_mat.gemm_view();
