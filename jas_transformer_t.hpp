...
/**
 * Decoder-only 基座：单路因果栈（无 encoder / cross-attn）。
 * 用法：embedding → forward / forward_one → lm_head → CE(next token)。
 */
template <typename input_type, template <typename> class updator_type>
class decoder_only_base_t
{
public:
    using val_type = typename input_type::ele_type;
    using kernel_type = decoder_only_t<val_type, updator_type>;
...
public:
    decoder_only_base_t(size_t n_layers = 1, size_t head_num = 1, int d_model = 1, int d_ff = 0)
        : m_kernel(static_cast<int>(n_layers), static_cast<int>(head_num), d_model, d_ff ? d_ff : d_model * 4),
    ...
    void set_param(size_t n_layers, size_t head_num, int d_model, int d_ff = 0)
    {
        m_head_num = static_cast<int>(head_num);
        m_d_model = d_model;
        m_kernel.set_param(static_cast<int>(n_layers), static_cast<int>(head_num), d_model,
                           d_ff ? d_ff : d_model * 4);
        ensure_rope_registry();
    }
    ...
    std::string net_type(int const& indent = 0) const
    {
        ...
        ss << print_indent(indent) << "decoder_only_base_t = [\n"
